<div class="row column spray-gun">
	<div class="sop-header row">
		<h3 class="text-center">Spray Gun Cleaning</h3>
		<div class="body">
			<div class="row">
				<div class="medium-2 column">
					Products Needed:
				</div>
				<div class="medium-4 column">
					<ul class="no-bullet">
						<li><span>U-POL # 2002</span>- Water-Based Cleaner</li>
						<li><span>SATA # 6007</span>- Cleaning Brushes</li>
						<li><span>SATA # 9209</span>- Small Cleaning Brushes</li>
					</ul>
				</div>
				<div class="medium-4 column">
					<ul class="no-bullet">
						<li><span>SATA # 9894</span>- Nozzle Cleaning Needles</li>
						<li><span>SATA # 38173</span>- Gun Lube</li>
						<li><span>Tork # 440278A</span>- Wipers</li>
					</ul>
				</div>
				<div class="medium-2 column">
					<div class="text-center safety">
						SAFETY<br>
						Safety Glasses<br>
						Organic Vapors Respirator<br>
						Disposable Gloves
					</div>
				</div>
			</div>
			<small class="float-right">Refer to the Manufacturer's MSDS for recommended safety equipment.</small>
		</div>
	</div>
	
	<div class="sop-body">
		<div class="row">
			<div class="medium-6 column">
				<h4>Disassembly of Nozzle Set</h4>
				<div class="row step">
					<div class="medium-2 column">Step #1</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-01.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Remove the paint needle.</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #2</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-02.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Remove the air cap.</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #3</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-03.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Unscrew fluid tip with a SATA gun wrench</p>
						</div>
					</div>
				</div>
				<h4>Cleaning and Drying</h4>
				<div class="row step">
					<div class="medium-2 column">Step #4</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-04.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Rinse / clean the material passage</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #5</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-05.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Rinse / clean the gun body</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #6</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-06.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Dry air passages and exterior of gun with an air blower</p>
						</div>
					</div>
				</div>
			</div>
			<div class="medium-6 column">
				<h4>Cleaning of the Nozzle Set</h4>
				<div class="row step">
					<div class="medium-2 column">Step #7</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-07.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Rinse / clean the air cap</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #8</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-08.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Use a cleaning brush to clean out the air cap</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #9</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-09.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Use a nozzle cleaning needle to clean out the smaller holes of the air cap</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #10</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-10.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Use cleaning brushes to clean the fluid tip</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Step #11</div>
					<div class="medium-3 column">
						<div class="border"><?=\Asset::img('paint_shop/06-11.jpg')?></div>
					</div>
					<div class="medium-7 column">
						<div class="border">
							<p>Rinse / clean the needle</p>
						</div>
					</div>
				</div>
				<div class="row step">
					<div class="medium-2 column">Info</div>
					<div class="medium-2 column">
						<div class="border"><?=\Asset::img('paint_shop/06-info.jpg')?></div>
					</div>
					<div class="medium-8 column">
						<div class="border qr">
							<div class="row column tech-support">
								Tech Support: <?=\Html::anchor('http://www.satausa.com','www.satausa.com');?><br>
								1-800-234-3876<br>
							</div>
							<div class="row column">
								<div class="float-left">
									QR Code:<br>
									Gun Cleaning Video
								</div>
								<?=\Asset::img('paint_shop/06-qr.jpg', array('class' => 'float-right'))?>
							</div>
						</div>
					</div>
				</div>
			</div>
		</div>
		<div class="row">
			<small><em>The suggested procedure on this page is a summary of the manufacturer's procedure. For complete details, <br><strong>review the manufacturer's instructions that accompany the products and/or that can be found at their websites.</strong></em></small>
		</div>
	</div>
</div>